...
- [STARsolo](#starsolo)
- [Simpleaf \& qcatch](#simpleaf--qcatch)
...
- `${meta.id}/qcatch/${meta.id}_qcatch_report.html`
  - The QC report generated by qcatch for each sample, unless `--skip_qcatch` is set.
- `${meta.id}/qcatch/${meta.id}_filtered_quants.h5ad`
  - Filtered qcatch quantifications in h5ad format.
- `${meta.id}/qcatch/${meta.id}_metrics_summary.csv`
  - Summary metrics exported by qcatch.
- `simpleaf_index`
...
> Some aligners, like `simpleaf` do not produce both raw&filtered matrices. When aligners give only one output, they are treated with the `raw` or `filtered` suffix accordingly. For `simpleaf`, by default only `raw` results are generated, followed by cellbender for filtering empty-drops to generate `cellbender_filter` results.
