...
workflow CELLRANGER_MULTI_ALIGN {
    ...
    ch_fastq
        ...
        .branch { meta, fastq ->
            ...
            if ((fastq == file("${projectDir}/assets/EMPTY", checkIfExists: true)) || params.fb_reference) {
                // when empty, should not check for reference
                return [meta, fastq]
            }
            else {
    ...
    if (!cellranger_vdj_index) {
        ...
        if (!params.skip_cellrangermulti_vdjref) {
            // if user uses cellranger multi but does not have VDJ data
            // Make reference genome
            CELLRANGER_MKVDJREF(
                ch_fasta,
                CELLRANGER_MKGTF.out.gtf,
    ...
    //
    CELLRANGER_MULTI(
        ch_grouped_fastq.gex.map { it -> it[0] },
        ch_grouped_fastq.gex,
...
def parse_demultiplexed_output_channels(in_ch, pattern) {
    def out_ch = in_ch
        .map { meta, mtx_files ->
            def desired_files = []
            mtx_files.each { it ->
                if (it.toString().contains("${pattern}")) {
        ...
        .map { meta, mtx_files ->
            ...
            // add metadata for conversion workflow
        ...
        .groupTuple(by: 0)
    // group it back as one file collection per sample
