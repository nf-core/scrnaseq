...
workflow CELLRANGERARC_ALIGN {
    ...
    main:
        ...
        if (!cellranger_index) {
            ...
            GENERATECONFIG(fasta.name, filtered_gtf.name, motifs.name)
            ch_versions.mix(GENERATECONFIG.out.versions)

            // Make reference genome
            CELLRANGERARC_MKREF( fasta, filtered_gtf, motifs, GENERATECONFIG.out.config, "cellranger_arc_reference" )
